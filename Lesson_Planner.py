...
def Import_Files(Input): #This function will examine all game files in the current directory and extract the useful information from them. It returns a dictionary of indexable data.
	Folder = sys.path[0]
	entries = os.listdir(Folder)
	data = Input	#Initilizes a dictionary that is stores all the data.
...
with open('directory.csv', 'w') as csv_file:
	...
	#for entry in (Info2):
	#	for i in range(len(Info2[entry])):
	#		writer.writerow(Info2[entry][i])#read this data into a CSV file. Also remove bogus data and let the user know if there is a bad file.